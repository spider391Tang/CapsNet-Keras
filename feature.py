...
# print 'Load train file ', train_file
...
for audio_filename in os.listdir(audio_folder):
    ...
    if is_mono:
        ...
        for ch in range(y.shape[0]):
            ...
            # sig, fs = librosa.load(audio_file)   
            # make pictures name 
            # save_path = 'test.png'
            ...
            # librosa.display.specshow(mbe_ch)
            # pylab.savefig(save_path, bbox_inches=None, pad_inches=0)
            # pylab.close()
            # plt.figure()
            # ax = plt.subplot(2,1,2)
            # librosa.display.specshow(librosa.power_to_db(mel_basis, ref=np.max))
    ...
    librosa.display.specshow(mbe, x_axis='linear')
